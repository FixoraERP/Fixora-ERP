...
export default function Page() {
  ...
  async function load(cid=s?.company_id){ if(!cid)return; const {data}=await supabase.from("clients").select("*").eq("company_id",cid).order("created_at",{ascending:false}); setRows(data||[]); }
  ...
  async function save(){ if(!s)return; if(!form.name)return alert("Informe o nome."); const p={...form,company_id:s.company_id,updated_at:new Date().toISOString()}; const r=id?await supabase.from("clients").update(p).eq("id",id):await supabase.from("clients").insert(p); if(r.error)return alert(r.error.message); setId(""); setForm(empty); load(); }
  async function del(){ if(!id)return alert("Selecione um registro."); if(!confirm("Excluir?"))return; await supabase.from("clients").delete().eq("id",id); setId(""); setForm(empty); load(); }
  return <div className="space-y-6"><h1 className="text-3xl font-black">Clientes</h1><section className="card p-4 grid md:grid-cols-3 gap-3">
